...
import {
  readymadeServerConfiguration,
  TransmitServerConfiguration,
} from './config.js';

const config: TransmitServerConfiguration = readymadeServerConfiguration;
...
if (config.osc) {
  ...
  touchOSCWSS.on('connection', (ws) => {
    // This is the format of osc message
    // {
    //   address: '/osc/address',
    //   args: [message],
...
oscServer.listen(touchOSCWSPort, hostname, () => {
  const address = chalk.underline.blue(
    `${protocol}://${hostname}:${touchOSCWSPort}`,
  );
  console.log(
    chalk.gray(
      `${chalk.white('TouchOSC Message Server')} is listening on ${address}`,
    ),
  );
});
...
signalServer.listen(signalPort, hostname, () => {
  const address = chalk.underline.blue(
    `${protocol}://${hostname}:${signalPort}`,
  );
  console.log(
    chalk.gray(`${chalk.white('Signal Server')} is listening on ${address}`),
  );
});

announceServer.listen(announcePort, hostname, () => {
  const address = chalk.underline.blue(
    `${protocol}://${hostname}:${announcePort}`,
  );
  console.log(
    chalk.gray(`${chalk.white('Announce Server')} is listening on ${address}`),
  );
});

messageServer.listen(messagePort, hostname, () => {
  const address = chalk.underline.blue(
    `${protocol}://${hostname}:${messagePort}`,
  );
  console.log(
    chalk.gray(`${chalk.white('Message Server')} is listening on ${address}`),
  );
});
...
app.listen(httpPort, () => {
  const address = chalk.underline.blue(`${protocol}://${hostname}:${httpPort}`);
  console.log(
    chalk.gray(`${chalk.white('Express Server')} is listening on ${address}`),
  );
  const iceAddress = chalk.underline.blue(
    `${protocol}://${hostname}:${httpPort}/ice`,
  );
  console.log(
    chalk.gray(
      `${chalk.white('Free ICE servers')} available by making a GET request to ${iceAddress}`,
    ),
  );
});
